...
const TypeIcon = ({ type }: { type: TravelDeal['type'] }) => {
  switch (type) {
    ...
    case 'activity': return <Ticket className="h-4 w-4 text-primary" />; // Using Ticket, could be ActivitySquare or FerrisWheel too
    case 'package': return <Briefcase className="h-4 w-4 text-primary" />; // Using Briefcase for package
